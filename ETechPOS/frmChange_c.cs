        ...
        private void frmChange_c_Shown(object sender, EventArgs e)
        {
            if (Visible && flag)
            {
                ...
                do
                {
                    ...
                    if (temp == 0)
                    {
                        LogsHelper.WriteToTLog("Tender success");
                    }
                    else if (temp == 1)
                    {
                        fncFilter.alert("Member transaction was unable to complete due to connection problems. F8 - Payment to try again.");
                        LogsHelper.WriteToTLog("Tender failed: Member Feature Offline");
                        isTransactionDone = false;
                    }
                    else if (temp == -1)
                    {
                        LogsHelper.WriteToTLog("Tender failed: General Saving failure");
                        ...
                        {
                            retry = true;
                            LogsHelper.WriteToTLog("Tender failure popup: Retry");
                        }
                        else
                        {
                            LogsHelper.WriteToTLog("Tender failure popup: Ignore");
